/**
 * Transcribe a single audio file using Local Whisper with CUDA acceleration
 *
 * Usage:
 *   npx tsx transcribe-file-cuda.ts <path-to-wav-file>
 *
 * Example:
 *   npx tsx transcribe-file-cuda.ts "./recordings/session-123/user_audio.wav"
 *
...
async function main() {
  try {
    ...
    if (!filePath) {
      console.error('❌ Usage: npx tsx transcribe-file-cuda.ts <path-to-wav-file>');
      console.error('\nExample:');
      console.error('  npx tsx transcribe-file-cuda.ts "./recordings/session-123/user_audio.wav"');
      process.exit(1);
    }
    ...
    console.log('🎙️  Local Whisper File Transcription (CUDA)\n');
    console.log('📂 Input:');
    ...
    console.log(`   Acceleration: CUDA (NVIDIA GPU)`);
    console.log('');

    // Create Whisper service with CUDA
    ...
    // Estimate time (with GPU, much faster!)
    const audioDurationSec = (stats.size / (1024 * 1024)) * 6; // ~6 seconds per MB for WAV
    const estimatedSec = Math.ceil(audioDurationSec / 15); // ~15x real-time on 4090
    console.log(`⏱️  Estimated time: ~${estimatedSec}s (GPU accelerated)\n`);
    ...
    const transcriptContent = [
      ...
      `Speed: ${(transcript.duration / (duration / 1000)).toFixed(1)}x real-time`,
      `Engine: CUDA (GPU Accelerated)`,
      '',
      '='.repeat(80),
      'CONTENT',
      '='.repeat(80),
    ...
    // Results
    const speedMultiplier = transcript.duration / (duration / 1000);
